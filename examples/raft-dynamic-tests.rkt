...
(module+ test
  ...
  (define (make-initialized-follower)
    ...
    (async-channel-put server (variant Config (record [members (list peer1 peer2 server)])))
  ...
  (define (make-peer1-follower-in-term term)
    ...
    (async-channel-put server (variant RequestVote term peer1 0 0))
  ...
  (define (make-leader-in-term target-term)
    ...
    (async-channel-put server (variant VoteCandidate target-term peer1))
...
(module+ test
  ...
  (test-case "Follower votes and resets election deadline if candidate has later term"
    ...
    (check-unicast peer1 (variant VoteCandidate 1 server))
  ...
  (test-case "Follower declines vote if candidate has earlier term"
    ...
    (async-channel-put server (variant RequestVote 1 peer2 1 1))
    (check-unicast peer2 (variant DeclineCandidate 2 server)))
  ...
  (test-case "Follower votes for at most one candidate per term"
    ...
    (async-channel-put server (variant RequestVote 1 candidate1 1 1))
    (check-unicast candidate1 (variant VoteCandidate 1 server))
    (define candidate2 (make-async-channel))
    (async-channel-put server (variant RequestVote 1 candidate2 1 1))
    (check-unicast candidate2 (variant DeclineCandidate 1 server)))

  (test-case "Follower can vote for same candidate multiple times in a term"
    (match-define (list server _ _ _ _ _) (make-initialized-follower))
    (define candidate (make-async-channel))
    (async-channel-put server (variant RequestVote 1 candidate 1 1))
    (check-unicast candidate (variant VoteCandidate 1 server))
    (async-channel-put server (variant RequestVote 1 candidate 1 1))
    (check-unicast candidate (variant VoteCandidate 1 server)))
  ...
  (test-case "Followers should check if candidate's log is up to date before granting vote"
    ...
    (check-unicast peer2 (variant AppendSuccessful 2 1 server))
    (async-channel-put server (variant RequestVote 3 peer1 0 0))
    (check-unicast peer1 (variant DeclineCandidate 3 server)))
  ...
  (test-case "Follower starts an election after timeout"
    ...
    (check-unicast-match peer1 (csa-variant RequestVote 1 (== server) 0 0))
    (check-unicast-match peer2 (csa-variant RequestVote 1 (== server) 0 0))
  ...
  (test-case "Follower ignores VoteCandidate messages"
    ...
    (tick)
    (async-channel-put server (variant RequestVote 4 peer1 3 0))
    (check-unicast peer1 (variant VoteCandidate 4 server)))

  (test-case "Follower ignores DeclineCandidate messages"
    (match-define (list server peer1 _ _ _ _) (make-peer1-follower-in-term 3))
    (async-channel-put server (variant DeclineCandidate 1 peer1))
    (tick)
    (async-channel-put server (variant RequestVote 4 peer1 3 0))
    (check-unicast peer1 (variant VoteCandidate 4 server)))
  ...
  (test-case "Follower ignores AppendRejected messages"
    (match-define (list server peer1 _ _ _ _) (make-peer1-follower-in-term 3))
    (async-channel-put server (variant AppendRejected 1 0 peer1))
    (tick)
    (async-channel-put server (variant RequestVote 4 peer1 3 0))
    (check-unicast peer1 (variant VoteCandidate 4 server)))
  ...
  (test-case "Follower ignores AppendSuccessful messages"
    (match-define (list server peer1 _ _ _ _) (make-peer1-follower-in-term 3))
    (async-channel-put server (variant AppendSuccessful 1 0 peer1))
    (tick)
    (async-channel-put server (variant RequestVote 4 peer1 3 0))
    (check-unicast peer1 (variant VoteCandidate 4 server)))
  ...
  (test-case "Follower responds to heartbeat messaages"
    ...
    (check-unicast peer1 (variant AppendSuccessful 1 0 server)))
  ...
  (test-case "Follower should commit each result as it gets new commit index from leader"
    ...
    (check-unicast peer1 (variant AppendSuccessful 1 1 server))
  ...
  (test-case "Follower rejects inconsistent updates"
    ...
    (check-unicast peer1 (variant AppendRejected 3 0 server)))
  ...
  (test-case "Follower does not update current term if leader is lagging"
    ...
    (check-unicast peer2 (variant AppendRejected 3 0 server))
    ;; check that term is still 3
    (async-channel-put server (variant RequestVote 2 peer2 0 0))
    (check-unicast peer2 (variant DeclineCandidate 3 server))))
...
;;     (match-define (list server peer1 peer2 _ _ _ _) (make-candidate-in-term 2))
;;     (async-channel-put server (variant DeclineCandidate 2 peer2))
;;     (async-channel-put server (variant VoteCandidate 2 peer1))
;;     (define heartbeat-message
;;       (variant AppendEntries 2 0 0 (list) 0 server server))
;;     (check-unicast peer1 heartbeat-message)
...
;;     (define peer1 (make-async-channel))
    ;; (async-channel-put server (variant Config [members (list server peer1 junk-channel junk-channel junk-channel)]))
;;     (define timeout-id (check-unicast-match timer-manager (csa-variant SetTimer _ _ id _ _) #:result id))
;;     (async-channel-put server (variant Timeout timeout-id))
...
;;     (match-define (list server peer1 _ _ _ _ _) (make-candidate-in-term 2))
;;     (async-channel-put server (variant RequestVote 3 peer1 0 0))
;;     (check-unicast peer1 (variant VoteCandidate 3 server))
;;     (tick)
;;     (async-channel-put server (variant RequestVote 3 peer1 0 0))
;;     (check-unicast peer1 (variant VoteCandidate 3 server)))
...
;;     (async-channel-put server (variant RequestVote 4 peer2 0 0))
;;     (check-unicast peer2 (variant VoteCandidate 4 server)))
...
;;     (async-channel-put server (variant Timeout timeout-id))
;;     (check-unicast server (variant RequestVote 2 server 0 0))
;;     (check-unicast server (variant RequestVote 2 server 0 0))
...
;;     (check-unicast client-response (LeaderIs (NoLeader))))
...
;;     (async-channel-put server (variant Timeout timeout-id))
;;     (check-unicast server (variant RequestVote 3 server 1 1))
;;     (async-channel-put server (variant RequestVote 4 peer1 0 0))
;;     (check-unicast peer1 (variant DeclineCandidate 4 server))))

;; ---------------------------------------------------------------------------------------------------
;; Leader tests

;; (module+ test
;;   (test-case "Leader periodically async-channel-puts heartbeat"
;;     (match-define (list server peer1 peer2 _ _ _ _) (make-leader-in-term 2))
;;     (async-channel-put (: server async-channel-put-heartbeat-timeouts) 1) ; timeout ID doesn't matter here
;;     (define heartbeat
;;       (variant AppendEntries 2 0 0 (list) 0 server server))
...
;;     ;; 1. someone else is leader and async-channel-puts a message
...
;;     (async-channel-put server (variant Timeout timeout-id))
;;     (check-unicast server (variant RequestVote 2 server 1 1))
;;     (async-channel-put server (variant VoteCandidate 2 peer1))
...
;;     (define timeout-id (check-unicast-match timer-manager (csa-variant SetTimer _ _ id _ _) #:result id))
;;     (async-channel-put server (variant Timeout timeout-id))
;;     (check-unicast server (variant RequestVote 2 server 1 5))
;;     (async-channel-put server (variant VoteCandidate 2 peer1))
...
;;     (define client (make-async-channel))
;;     (async-channel-put server  (variant ClientMessage client "a"))
;;     (check-unicast client (LeaderIs (NoLeader)))))
