...
(define-variant MiscCommands
  ...
  ;; DeathWatch messages (these model the Terminated messages generated by the Akka framework)
  (TaskManagerTerminated [id TaskManagerId])
  (JobManagerTerminated)
  (CancelTask [id JobTaskId]))
...
(define-actor TaskRunnerInput
  ...
  (define-state (AwaitingTask) (m)
    (case m
      ...
      [(CancelTask id) (goto AwaitingTask)]
      ;; TODO: other commands
      ))
  (define-state/timeout (AboutToRunTask [task ReadyTask]) (m)
    (case m
      ...
      [(CancelTask to-cancel-id)
       (cond
         [(= to-cancel-id (: task id)) (goto AwaitingTask)]
  ...
  (define-state (WaitingForNextSplit [id JobTaskId] [word-count (Hash String Nat)]) (m)
    (case m
      ...
      [(CancelTask to-cancel-id)
       (cond
         [(= to-cancel-id id) (goto AwaitingTask)]
      ...
      [(JobManagerTerminated)
       (let ([idle-runners
              (for/fold ([idle-runners idle-runners])
                        ([busy-runner busy-runners])
                (send (: busy-runner addr) (CancelTask (: busy-runner id)))
